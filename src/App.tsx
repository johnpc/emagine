...
const MovieCard = (props: { movie: Movie; showtimeInfo: ShowtimeInfo[] }) => {
  ...
  const ratings: string[] = omdbInfo?.ratings ? Object.values(omdbInfo?.ratings).map((r) => `${r.source}: ${r.value}`) : [];
      ...
      <Card>
        <Flex direction="row" alignItems="flex-start">
          ...
          >
            ...
            {Object.values(omdbInfo?.actors ?? {}).length ? <>Actors:
              {Object.values(omdbInfo?.actors ?? {}).map(actor => <li>{actor}</li>)}
            </> : null }
            {Object.values(omdbInfo?.genre ?? {}).length ? <>Genre:
              {Object.values(omdbInfo?.genre ?? {}).map(genre => <li>{genre}</li>)}
            </> : null }
          </Flex>
        </Flex>
      </Card>
